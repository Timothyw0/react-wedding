const englishTextPassword = {
  welcome: "Welcome!",
  pass: "Please enter the password to continue:",
  incorrect: "The password is incorrect! Please try again.",
  enter: "Enter",
};

const chineseTextPassword = {
  welcome: "欢迎!",
  pass: "请输入密码",
  incorrect: "密码不正确！ 请再试一次。",
  enter: "进入",
};

const englishTextHome = {
  tim: "Timothy Wang",
  madeleine: "Madeleine Herring",
  date: "September 23, 2023",
  city: "Warrington, PA",
  days: "Days",
  day: "Day",
};

const chineseTextHome = {
  tim: "王大明",
  madeleine: "沛欣",
  date: "2023 年 9 月 23 日 (星期六)",
  city: "Warrington, PA",
  days: "天",
  day: "天",
};

const englishTextNav = {
  pages: [
    "Home",
    "Our Story",
    "Wedding Party",
    "Events",
    "Accommodations",
    "RSVP",
    "Registry",
    "Q & A",
  ],
  pageLinks: {
    Home: "/",
    "Our Story": "/story",
    "Wedding Party": "/party",
    Events: "/events",
    Accommodations: "/accommodations",
    RSVP: "/rsvp",
    Registry: "/registry",
    "Q & A": "/questions",
  },
  changeLanguage: "Change Language",
  languageMenu: ["English", "中文"],
};

const chineseTextNav = {
  pages: [
    "主页",
    "故事",
    "婚礼派对",
    "婚礼程序",
    "住宿",
    "敬请注册",
    "礼物登记处",
    "问答",
  ],
  pageLinks: {
    主页: "/",
    故事: "/story",
    婚礼派对: "/party",
    婚礼程序: "/events",
    住宿: "/accommodations",
    敬请注册: "/rsvp",
    礼物登记处: "/registry",
    问答: "/questions",
  },
  changeLanguage: "改变语言",
  languageMenu: ["English", "中文"],
};

const englishTextDial = {
  email: "Email Us",
  text: "Text Us",
  code: "Check Out My Code!",
};

const chineseTextDial = {
  email: "电子邮箱",
  text: "短信",
  code: "我的代码!",
};

const englishText404 = {
  text: "Page not found! Please choose a link from the topbar.",
};

const chineseText404 = {
  text: "网页未找到！ 请从顶部栏中选择一个链接。",
};

const englishTextTravel = {
  header: "Hampton Inn Doylestown - Warrington",
  addressText: "Address",
  address: "1570 Easton Rd, Warrington, PA 18976",
  phoneText: "Phone",
  phone: "(215) 343-8400",
  venue: "Wedding Venue",
  hotel: "Hampton Inn",
  hotelURL:
    " https://www.hilton.com/en/book/reservation/deeplink/?ctyhocn=DYLPAHX&groupCode=CHHHWW&arrivaldate=2023-09-23&departuredate=2023-09-24&cid=OM,WW,HILTONLINK,EN,DirectLink&fromId=HILTONLINKDIRECT",
  hotelText: "Hotel Website",
};

const chineseTextTravel = {
  header: "Hampton Inn Doylestown - Warrington",
  addressText: "地址",
  address: "1570 Easton Rd, Warrington, PA 18976",
  phoneText: "电话",
  phone: "(215) 343-8400",
  venue: "婚礼场地",
  hotel: "酒店",
  hotelURL:
    " https://www.hilton.com/en/book/reservation/deeplink/?ctyhocn=DYLPAHX&groupCode=CHHHWW&arrivaldate=2023-09-22&departuredate=2023-09-24&cid=OM,WW,HILTONLINK,EN,DirectLink&fromId=HILTONLINKDIRECT",
  hotelText: "酒店网站",
};

const englishTextStory = {
  header: "Our Story",
  story: `Tim and Madeleine met in 2019 during their senior year of college. Tim studied at RPI and Madeleine studied at Russell Sage College, both in Troy, NY. They met at a bar called “The Ruck” and hit it off from there. After graduation, Tim started his job in Jersey City, while Madeleine attended graduate school at Fordham University. Three years later... on March 25, 2022 Tim proposed in Boston, Massachusetts. They currently live in Jersey City, and are as happy as ever!`,
};

const chineseTextStory = {
  header: "我们的故事",
  story:
    "2019年的初春我们在纽约州特洛伊的一个名为“The Ruck”的酒吧相遇，那时的我们彼此倾心一见钟情。当时Tim在RPI读书，而Maddy则是“Russell Sage ”学院的一名学生并正在研修一门中文课程。两校在同一区域且相隔不远，从此开启了爱情生活模式。当年毕业后Tim 入职于“美国银行”并在新泽西州的泽西市开始了他的新生活，Maddy则在纽约州的“复旦大学”攻读研究生。三年后……2022年3月25日Tim在双方好友的见证下在马塞诸塞州波士顿求婚。现在我们一起居住在新泽西州的泽西市享受着我们平淡而甜蜜的生活。",
};

const englishTextRegistry = {
  registryURL: "https://www.zola.com/registry/madeleineandtimothy2023",
  header: "Thank you for thinking of us, please find our registry below!",
  registryText: "Registry",
};

const chineseTextRegistry = {
  registryURL: "https://www.zola.com/registry/madeleineandtimothy2023",
  header: "您可以在下面认购我们所需要的礼物：",
  registryText: "礼物登记处",
};

const englishTextQA = {
  questions: [
    "What is the dress code?",
    "What is the parking situation?",
    "I have dietary restrictions, how should I let you know?",
    "What hotel should I book?",
    "What is the best way to contact you?",
    "What is your wedding hashtag?",
  ],
  answers: [
    "The dress code for our wedding is semi-formal/cocktail attire",
    "There is complimentary parking on-site and the Hampton Inn is less than a 3 minute drive away if you are using Uber/Lyft",
    "Please include any dietary restrictions in the RSVP",
    "We have reserved rooms at the Hampton Inn Doylestown nearby, you can check out their website",
    "You can text Madeleine at (845)-820-3221 or Tim at (732)-939-3051",
    "#TwoWangsMakeARight",
  ],
  rsvpText: "here",
  hereText: "here",
};

const chineseTextQA = {
  questions: [
    "着装要求是什么？",
    "停车情况如何？",
    "如何提交食物过敏的信息？",
    "我应该预订什么酒店？",
    "与您联系的最佳方式是什么？",
    "你的婚礼标签是什么？",
  ],
  answers: [
    "我们婚礼的着装要求是正装出席。",
    "婚礼场所提供免费停车场。",
    "请在注册时填写何种食物过敏。",
    "我们推荐附近的 Hampton Inn Doylestown，您可以查看他们的网站 ",
    "发短信至：Madeleine (845)-820-3221 或者 王大明 (732)-939-3051",
    "#TwoWangsMakeARight",
  ],
  rsvpText: "注册",
  hereText: "这里",
};

const englishTextParty = {
  moh: "Maid of Honor",
  maoh: "Matron of Honor",
  groomsman: "Groomsman",
  bridesmaid: "Bridesmaid",
  bestman: "Best Man",
  party: "Wedding Party",
  kim: "Sister of the Bride",
  emily: "Sister of the Bride",
  theresa: "Sister of the Groom",
  donny: "Brother of the Bride",
};

const chineseTextParty = {
  moh: "伴娘",
  maoh: "伴娘",
  groomsman: "伴郎",
  bridesmaid: "伴娘",
  bestman: "伴郎",
  party: "婚礼派对",
  kim: "新娘的姐姐",
  emily: "新娘的妹妹",
  theresa: "新郎的妹妹",
  donny: "新娘的弟弟",
};

const englishTextRSVP = {
  header: "We hope that you can make it!",
  subheader: "Please enter your information below to RSVP",
  cannotAttend: "I cannot attend",
  guests: "# of Guests",
  email: "Email*",
  firstname: "First Name*",
  lastname: "Last Name*",
  dietary: "Dietary Restrictions",
  food: "Food Selection*",
  foodChoices: [
    "Brazilian Charcoal Steak",
    "Chicken Warrington",
    "Sesame Salmon",
    "Almshouse Pasta",
  ],
  foodTitles: [
    "Brazilian Charcoal Steak",
    "Chicken Warrington",
    "Sesame Salmon",
    "Almshouse Pasta",
  ],
  foodDescriptions: [
    "Brazilian steak sauce, chimichurri & char-grilled onions",
    "Organic spinach & boursin cheese, champagne chive butter sauce",
    "Black plum & sweet chili glaze, sesame seed crust, wonton ribbons",
    "Campanelle, seasonal vegetables, tomato-basil cream, parmesan",
  ],
  showFoodButton: "Food Details",
  songRequests: "Any Song Requests?",
  songRequestLabel: "Requests",
  error: "Something went wrong! Please tell Tim!",
  success: "You have RSVP'd! Can't wait to see you!",
  missingEmail: "Please enter your email!",
  alreadyRSVP: "You are already RSVP'd!",
  thanks: "Thanks for letting us know! Take care!",
  thanksRSVP: "Thank you for RSVP'ing!",
  checkFieldMessage: "Please fill out all required fields!",
  rsvpButton: "RSVP!",
  backButton: "Back",
  nextButton: "Next",
  guestPrefix: "Guest",
  attendanceInfo: "Attendance Info",
  review: "Review",
  rsvpNote:
    "Note: You will not be able to change your RSVP after submission. If anything changes, please contact us!",
};

const chineseTextRSVP = {
  header: "我们真诚地希望您能前来参加婚礼！",
  subheader: "请在下方输入您的信息",
  cannotAttend: "抱歉，我不能出席",
  guests: "参加人数",
  email: "Email*",
  firstname: "名字*",
  lastname: "姓*",
  dietary: "食物过敏",
  food: "食物选择*",
  foodChoices: ["巴西木炭牛排", "沃灵顿鸡", "香芝三文鱼", "意大利面"],
  foodTitles: ["巴西木炭牛排", "沃灵顿鸡", "香芝三文鱼", "意大利面"],
  foodDescriptions: [
    "巴西烤肉酱，阿根廷（Chimichurri)风味酱汁&烤洋葱",
    "黑李甜辣酱，香脆芝麻，炸云吞柳",
    "有机菠菜&布尔辛（Boursin)奶酪，香葱奶油酱汁",
    "风铃面，应季蔬菜，西红柿-罗勒（Basil)酱汁，帕尔玛（Parmesan)奶酪",
  ],
  showFoodButton: "食物选择",
  songRequests: "点播歌曲",
  songRequestLabel: "点播",
  error: "出问题了！ 请告诉 Tim！",
  success: "你已经回复了！ 迫不及待地想见到你！",
  missingEmail: "请输入您的邮箱！",
  alreadyRSVP: "您已经注册了！",
  thanks: "感谢您让我们知道！ 小心！",
  thanksRSVP: "感谢您的回复！",
  checkFieldMessage: "请填写所有必要的信息。",
  rsvpButton: "提交注册!",
  backButton: "返回",
  nextButton: "下一页",
  guestPrefix: "宾客",
  attendanceInfo: "宾客信息",
  review: "确认",
  rsvpNote:
    "您的信息确认以后不能在该系统中更改。如果您需要更改其中的任何信息请联系我们。",
};

const englishTextEvents = {
  events: "Events",
  rehearsal: "Rehearsal Dinner",
  invite: "(Invite Only)",
  rehearsalDate: "September 22, 2023 (Friday)",
  ceremony: "Ceremony",
  ceremonyDate: "September 23, 2023 (Saturday)",
  ceremonyArrive: "Please arrive 15 minutes prior",
  cocktail: "Cocktail Hour",
  reception: "Reception",
  afterparty: "Afterparty",
  viewMap: "View on Map ",
};

const chineseTextEvents = {
  events: "婚礼程序",
  rehearsal: "晚宴彩排",
  invite: "(仅限邀请人员)",
  rehearsalDate: "2023 年 9 月 22 日 (星期五)",
  ceremony: "婚礼",
  ceremonyDate: "2023 年 9 月 23 日 (星期六)",
  ceremonyArrive: "请提前15分钟到达",
  cocktail: "鸡尾酒会",
  reception: "婚宴",
  afterparty: "后派对",
  viewMap: "地图 ",
};

module.exports = {
  englishTextPassword,
  chineseTextPassword,
  englishTextHome,
  chineseTextHome,
  englishTextNav,
  chineseTextNav,
  englishTextDial,
  chineseTextDial,
  englishText404,
  chineseText404,
  englishTextTravel,
  chineseTextTravel,
  englishTextStory,
  chineseTextStory,
  englishTextRegistry,
  chineseTextRegistry,
  englishTextQA,
  chineseTextQA,
  englishTextParty,
  chineseTextParty,
  englishTextRSVP,
  chineseTextRSVP,
  englishTextEvents,
  chineseTextEvents,
};
